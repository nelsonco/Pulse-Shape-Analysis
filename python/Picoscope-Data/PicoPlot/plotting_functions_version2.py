...
class picoscope:
    ...
    def __init__(self,channel_number, duration, noSamples,qwmin, twmin, wmax):
            self.qwmin = qwmin # The index of the first voltage after trigger is 50 this needs to be factored into the window calculation
            self.twmin = twmin
            self.wmax = wmax
    ...
    def Q_tail(self, channel_number, dt, directory,date_list):
            a = 0
            self.q_tail = []
            while a < len(self.data[:,0]):
                tsum = np.sum(self.data[a,self.twmin:self.wmax])
                self.q_tail = np.append(self.q_tail,tsum*dt)
                a =a + 1
            print("q_tail = ", self.q_tail)

    def Q_tot(self,channel_number, dt, date_list, directory):
            i = 0
            self.q_tot = []
            self.q_tot_index = []
            while i < len(self.data[:,0]):
                qsum = np.sum(self.data[i, self.qwmin:self.wmax])
    ...
    def qt_scatter(self, band, gamma_band, q_selection, date_list, directory, time_elapsed,ax=None, addDir = False):
        ...
        x6 = 1.72e-8
        y6 = 7.1e-8
        ax1.plot([x1,x2],[y1,y2], c="black")
        ax1.plot([x3,x4], [y3,y4], c = "black")
        ax1.plot([x5,x6], [y5,y6], c = "red")
        n = 0
        m=0
        i = 0
        ...
        while i<len(self.data[:,0]):
            ...
            elif abs((y4-y3)/(x4-x3))<abs(self.q_tot[i]/self.q_tail[i])<abs((y2-y1)/(x2-x1)) or (abs(self.q_tail[i])<x3 and abs(self.q_tot[i]/self.q_tail[i])<abs((y2-y1)/(x2-x1))):
                n = n+1
                self.set_gamma_band_index=np.append(self.set_gamma_band_index, int(i))
            elif abs((y6-y5)/(x6-x5))<abs(self.q_tot[i]/self.q_tail[i])<abs((y4-y3)/(x4-x3)):
                m = m+1
                self.set_neutron_band_index=np.append(self.set_neutron_band_index, int(i))
            i = i+1
    
        print("Time elapsed =", time_elapsed)
        print("Total number of events = ", len(self.data[:,0]))
        print("rate = ", len(self.data[:,0])/time_elapsed)
        print("Gamma:")
        print("# of events in gamma band = ", n)
        print("rate = ", n/time_elapsed)
        print("Neutron:")
        print("# of events in neutron band = ",m)
        print("rate = ", m/time_elapsed)
